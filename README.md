...
All three questions above are about **paths and connectivity**, not about individual rows, and
that's exactly where a graph database earns its place over a relational one:
...
- **`GET /api/fraud-rings`**: the core ring-detection query:
  ...
  identifier used by more than one account. That's the multi-table self-join problem described
  above, solved as a single pattern match per identifier type.
...
That's expected on first load of the demo.
